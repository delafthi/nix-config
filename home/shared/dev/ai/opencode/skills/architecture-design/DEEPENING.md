...
Pure computation, in-memory state, no I/O. Always deepenable. Merge modules,
test through new interface directly. No adapter needed.

### 2. Local-substitutable

Dependencies with local test stand-ins (PGLite for Postgres, in-memory
filesystem). Deepenable if stand-in exists. Seam is internal; no port at
external interface.

### 3. Remote but owned

Your own services across network boundary. Define a **port** (interface) at
seam. Deep module owns logic; transport is injected **adapter**. Tests use
in-memory adapter. Production uses HTTP/gRPC/queue adapter.

### 4. True external

Third-party services you don't control. Take dependency as injected port; tests
...
- **Internal seams vs external seams.** Deep module can have internal seams
  ...
  internal seams through interface.
